...
4. In **Workers & Pages → holter-box-check → Settings → Variables and Secrets**, ensure `PASSCODE` and `IP_HASH_SECRET` are encrypted **Secret** values for Production. Set or change a secret in the dashboard; never put either secret in source, GitHub, a QR code, or a client build.
