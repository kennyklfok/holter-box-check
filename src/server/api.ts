...
function configured(env: Env): boolean {
  return Boolean(env.DB && env.PASSCODE && env.IP_HASH_SECRET);
}
...
export async function login(request: Request, env: Env): Promise<Response> {
  ...
  if (typeof data?.passcode !== 'string' || data.passcode.length > 256) return json({ error: 'Enter a valid passcode.' }, 400);
  try {
